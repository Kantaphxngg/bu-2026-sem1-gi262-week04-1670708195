...
namespace Assignment
{
    public class Assignment : MonoBehaviour
    {
        ...
        public void AS02_CountNumber()
        {
            ...
            int[] num = new int[numberCounts.Count];
            int[] counts = new int[numberCounts.Count];
 
            numberCounts.Keys.CopyTo(num, 0);
            numberCounts.Values.CopyTo(counts, 0);
 
            // แสดงผลลัพธ์
            for (int i = 0; i < num.Length; i++)
            {
                Debug.Log("number: " + num[i] + " count: " + counts[i]);
            }
        }
        ...
        public void AS11_PlayerStatsTracker()
        {
            ...
            Debug.Log(
                "Updated " + statName + ": " + playerStats[statName]
            );
